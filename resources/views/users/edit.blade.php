                    ...
                    <div class="card-body">
                        <form action="{{route('users.update', $user->id)}}" method="POST">
                            ...
                            <div class="form-group row mt-3">
                                ...
                                <div class="col-md-6">
                                `<input type="text" id="name" class="form-control" name="name" value="{{ $user->fullname }}" required autofocus>
                                    @if ($errors->has('name'))
                            ...
                            <div class="col-md-6">
                                <select class="form-select" id="role" name="role" aria-label="role">
                                    ...
                                    @foreach($roles as $val)
                                        <option value="{{$val->id}}" {{ ($user->role_name == $val->id) ? 'selected' : '' }}>{{$val->role_name}}</option>
                                    @endforeach
                                </select>
                                @if ($errors->has('role'))
